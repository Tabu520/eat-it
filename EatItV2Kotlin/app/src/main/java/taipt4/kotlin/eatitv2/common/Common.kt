...
import taipt4.kotlin.eatitv2.model.*
...
object Common {
    ...
    fun calculateExtraPrice(
        ...
        userSelectedAddonModel: MutableList<AddonModel>?
    ): Double {
        var result: Double = 0.0
        if (userSelectedSize == null && userSelectedAddonModel == null) {
            return 0.0
        } else if (userSelectedSize == null) {
            for (addonModel in userSelectedAddonModel!!) {
                result += addonModel.price.toDouble()
            }
            return result
        } else if (userSelectedAddonModel == null) {
    ...
    fun getDateOfWeek(input: Int): String {
        return when (input) {
            1 -> "Monday"
            2 -> "Tuesday"
            3 -> "Wednesday"
